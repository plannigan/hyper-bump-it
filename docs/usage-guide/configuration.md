...
* `tag_name_format_pattern` is a [format pattern][format-patterns] used to produce the name of the
    tag. If not specified, the default value of, the default value of `"v{new_version}"` is used.
* `tag_message_format_pattern` is a [format pattern][format-patterns] used to produce the message
    of the tag. If not specified, the default value of, the default value of
    `"Bump version: {current_version} → {new_version}"` is used.
